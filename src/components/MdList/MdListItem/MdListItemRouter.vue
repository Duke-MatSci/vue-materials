...
export default {
  ...
  props: {
    ...
    custom: Boolean,
    disabled: Boolean,
    mdRipple: { type: Boolean, default: true },
  },
  computed: {
    isDisabled() { return !this.mdRipple || this.disabled },
  },
  render() {
      ...
      },
      {
        default: ({ href, navigate, isActive, isExactActive }) =>
          h(
            ...
            {
              href,
              onClick: (e) => { this.disabled ? e.preventDefault() : navigate(e) },
              class: {
                "md-router-link": true,
