...
export default {
	...
	methods: {
		...
		addSelectableItem() {
			if (this.MdTable.selectable && this.mdItem) {
				const index = this.MdTable.selectable.indexOf(this.mdItem)
				if (index === -1) {
					this.MdTable.selectable.push(this.mdItem)
				}
			}
		},
		removeSelectableItem(item = this.mdItem) {
			if (this.MdTable.selectable && item) {
				const index = this.MdTable.selectable.indexOf(item)
				if (index > -1) {
					this.MdTable.selectable.splice(index, 1)
				}
			}
		},
	},
