...
export default {
  props: {
    mdMode: { type: String, ...MdPropValidator("md-mode", mdAppModes) },
    mdWaterfall: Boolean,
    mdScrollbar: { type: Boolean, default: true },
  },
  data: () => ({
    revealTimer: null,
    revealLastPos: 0,
    manualTick: false,
    MdApp: {
      options: { mode: null, waterfall: false, flexible: false },
      toolbar: {
        element: null,
        titleElement: null,
        height: "0px",
  ...
  computed: {
    isFixed() { return this.mdMode && this.mdMode !== "fixed" },
    isDrawerMini() {
    ...
    contentStyles() {
      return { ["padding-" + (this.MdApp.drawer.right ? "right" : "left")]: this.contentPadding }
    },
    containerStyles() {
      const styles = {}
      if (this.isFixed) styles["margin-top"] = this.MdApp.toolbar.initialHeight + "px"
      if (this.isDrawerMini) styles["padding-" + (this.MdApp.drawer.right ? "right" : "left")] = !this.MdApp.drawer.active ? this.MdApp.drawer.initialWidth + "px" : 0
      return styles
    },
    scrollerClasses() { return this.mdScrollbar ? "md-scrollbar" : undefined },
    appClasses() {
      return {
  ...
  watch: {
    mdMode(mode) { this.MdApp.options.mode = mode },
    mdWaterfall(waterfall) { this.MdApp.options.waterfall = waterfall; this.setToolbarElevation() },
  },
  methods: {
    setToolbarElevation() { this.MdApp.toolbar.hasElevation = !this.mdWaterfall },
    setToolbarTimer(scrollTop) {
      window.clearTimeout(this.revealTimer)
      this.revealTimer = window.setTimeout(() => { this.revealLastPos = scrollTop }, 100)
    },
    setToolbarMarginAndHeight(margin, height) { this.MdApp.toolbar.top = margin; this.MdApp.toolbar.height = height },
    getToolbarConstrants($event) {
      ...
      if (!this.MdApp.toolbar.initialHeight) this.MdApp.toolbar.initialHeight = toolbarHeight
      return { toolbarHeight, safeAmount, threshold, scrollTop, initialHeight: this.MdApp.toolbar.initialHeight }
    },
    handleWaterfallScroll($event) {
      let { threshold, scrollTop } = this.getToolbarConstrants($event)
      let elevationMark = 4
      if (this.mdMode === "reveal") elevationMark = threshold
    ...
    handleFlexibleMode($event) {
      ...
      if (firstRowHeight) {
        toolbar.style.height = (shouldKeepFlexible ? scrollAmount : firstRowHeight) + "px"
      }
    ...
    handleRevealMode($event) {
      ...
      this.setToolbarMarginAndHeight(scrollTop - threshold, toolbarHeight)
      this.MdApp.toolbar.revealActive = scrollTop < threshold ? true : this.revealLastPos > scrollTop + safeAmount
    },
    handleFixedLastMode($event) {
      ...
      this.MdApp.toolbar.fixedLastActive = scrollTop < firstRowHeight ? true : this.revealLastPos > scrollTop + 10
    },
    ...
    handleModeScroll($event) {
      if (this.mdMode === "reveal") this.handleRevealMode($event)
      else if (this.mdMode === "fixed-last") this.handleFixedLastMode($event)
      else if (this.mdMode === "overlap") this.handleOverlapMode($event)
      else if (this.mdMode === "flexible") this.handleFlexibleMode($event)
    },
    handleScroll($event) {
      if (this.MdApp.toolbar.element) {
        raf(() => {
          if (this.mdWaterfall) this.handleWaterfallScroll($event)
          if (this.mdMode) this.handleModeScroll($event)
        })
      }
    },
  },
  created() { this.MdApp.options.mode = this.mdMode; this.MdApp.options.waterfall = this.mdWaterfall; this.setToolbarElevation() },
  mounted() {
    const fakeEvent = { target: { scrollTop: 0 } }
    if (this.mdMode === "reveal") { this.MdApp.toolbar.revealActive = true; this.handleRevealMode(fakeEvent) }
    if (this.mdMode === "flexible") { this.MdApp.toolbar.revealActive = true; this.handleFlexibleMode(fakeEvent) }
    if (this.mdMode === "fixed-last") { this.MdApp.toolbar.fixedLastActive = true; this.handleFixedLastMode(fakeEvent) }
    if (this.mdMode === "overlap") { this.handleOverlapMode(fakeEvent) }
  },
}
