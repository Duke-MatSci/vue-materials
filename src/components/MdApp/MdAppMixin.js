...
export default {
	...
	computed: {
		...
		isDrawerMini() {
			return (
				this.MdApp.drawer.mode === "persistent" &&
				this.MdApp.drawer.submode === "mini"
			)
		},
		contentPadding() {
			...
			if (
				...
				this.MdApp.drawer.mode === "persistent" &&
				this.MdApp.drawer.submode === "full"
			) {
		...
		contentStyles() {
			return {
				[`padding-${this.MdApp.drawer.right ? "right" : "left"}`]:
					this.contentPadding,
			}
		},
		containerStyles() {
			...
			if (this.isDrawerMini) {
				styles[`padding-${this.MdApp.drawer.right ? "right" : "left"}`] = !this
					.MdApp.drawer.active
					? this.MdApp.drawer.initialWidth + "px"
					: 0
			}
	...
	methods: {
				...
				if (shouldKeepFlexible) {
					const newSize =
						Math.max(
							0,
							1 -
								(scrollTop - initialSize) /
									(scrollAmount + initialSize + 0.000001)
						) *
		...
		handleRevealMode($event) {
			const { toolbarHeight, safeAmount, threshold, scrollTop } =
				this.getToolbarConstrants($event)

			this.setToolbarTimer(scrollTop)
			this.setToolbarMarginAndHeight(scrollTop - threshold, toolbarHeight)

			if (scrollTop >= threshold) {
				this.MdApp.toolbar.revealActive =
					this.revealLastPos > scrollTop + safeAmount
			} else {
				this.MdApp.toolbar.revealActive = true
			}
		},
		handleFixedLastMode($event) {
			let { scrollTop, toolbarHeight, safeAmount } =
				this.getToolbarConstrants($event)
			const toolbar = this.MdApp.toolbar.element
			const firstRow = toolbar.querySelector(".md-toolbar-row:first-child")
			const firstRowHeight = firstRow.offsetHeight
			...
			if (scrollTop >= firstRowHeight) {
				this.MdApp.toolbar.fixedLastActive =
					this.revealLastPos > scrollTop + safeAmount
			} else {
		...
		handleOverlapMode($event) {
			const { toolbarHeight, scrollTop, initialHeight } =
				this.getToolbarConstrants($event)
			const toolbar = this.MdApp.toolbar.element
			const firstRow = toolbar.querySelector(".md-toolbar-row:first-child")
			const firstRowHeight = firstRow.offsetHeight
			...
			const newHeight =
				...
				scrollTop -
				(scrollTop * 100) /
					(initialHeight - firstRowHeight - firstRowHeight / 1.5)

			if (firstRowHeight) {
				if (
					scrollTop < initialHeight - firstRowHeight &&
					newHeight >= firstRowHeight
				) {
